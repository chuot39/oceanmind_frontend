...
import { useRestoreFaculty } from '../mutationHook';

const FacultyTable = ({ data, loading, pagination, onEdit, onDelete }) => {
    const intl = useIntl();

    const { mutate: restoreFaculty, isLoading: isRestoring } = useRestoreFaculty();
    ...
    const handleRestore = (documentId) => {
        return ModalConfirm({
            typeModal: 'restore_faculty',
            loading: isRestoring,
            handleResolve: async () => {
                try {
                    restoreFaculty({ documentId },
                        {
                            onSuccess: () => { },
                            onError: (error) => {
    ...
    const columns = [
        {
            title: getColumnTitle(<FormattedMessage id="admin.faculty.vn_name" defaultMessage="Vietnamese Name" />, 'name_vi'),
            dataIndex: 'name_vi',
            key: 'name_vi',
        ...
        {
            title: getColumnTitle(<FormattedMessage id="admin.faculty.en_name" defaultMessage="English Name" />, 'name_en'),
            dataIndex: 'name_en',
            key: 'name_en',
        ...
        {
            title: getColumnTitle(<FormattedMessage id="admin.faculty.created_at" defaultMessage="Created At" />, 'createdAt', true),
            dataIndex: 'createdAt',
            key: 'createdAt',
            width: 180,
        ...
        {
            title: getColumnTitle(<FormattedMessage id="admin.faculty.updated_at" defaultMessage="Updated At" />, 'updatedAt', true),
            dataIndex: 'updatedAt',
            key: 'updatedAt',
            width: 180,
        ...
        {
            title: <FormattedMessage id="admin.faculty.actions" defaultMessage="Actions" />,
            key: 'actions',
            width: 120,
            render: (_, record) => (
                <Space size="middle">
