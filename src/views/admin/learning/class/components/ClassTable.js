...
import { useRestoreClass } from '../mutationHook';
import useSkin from '@/utils/hooks/useSkin';

const ClassTable = ({ data, loading, pagination, onChange, onEdit, onDelete }) => {
    const intl = useIntl();
    const { language } = useSkin()

    const { mutate: restoreClass, isLoading: isRestoring } = useRestoreClass();
    ...
    const handleRestore = (documentId) => {
        return ModalConfirm({
            typeModal: 'restore_class',
            loading: isRestoring,
            handleResolve: async () => {
                try {
                    restoreClass({ documentId },
                        {
                            onSuccess: () => { },
                            onError: (error) => {
    ...
    const columns = [
        {
            title: getColumnTitle(<FormattedMessage id="admin.class.name" defaultMessage="Class Name" />, 'name'),
            dataIndex: 'name',
            key: 'name',
            render: (text) => <span className="font-medium">{text}</span>,
        },
        {
            title: <FormattedMessage id="admin.class.description" defaultMessage="Description" />,
        ...
        {
            title: <FormattedMessage id="admin.class.specialization" defaultMessage="Specialization" />,
            key: 'specialization',
            render: (_, record) => (
                <Tag className="tag-specialized">
                    {language === 'vi' ? record.specialized?.name_vi : record.specialized?.name_en || 'N/A'}
                </Tag>
            ),
        },
        {
            title: <FormattedMessage id="admin.class.batch" defaultMessage="Batch" />,
            key: 'batch',
            render: (_, record) => (
                <Tag className="tag-batch">
                    {record.batch?.name || 'N/A'}
                </Tag>
            ),
        },
        {
            title: getColumnTitle(<FormattedMessage id="admin.class.created_at" defaultMessage="Created At" />, 'createdAt', true),
            dataIndex: 'createdAt',
            key: 'createdAt',
            width: 180,
        ...
        {
            title: <FormattedMessage id="admin.class.actions" defaultMessage="Actions" />,
            key: 'actions',
            width: 120,
            render: (_, record) => (
                <Space size="middle">
