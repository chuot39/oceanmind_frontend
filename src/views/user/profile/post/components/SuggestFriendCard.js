...
const SuggestFriendCard = ({ skin, suggestFriends, userData }) => {
    const [isModalVisible, setIsModalVisible] = useState(false);

    console.log('friends', suggestFriends);
            ...
            >
                <div>
                    ...
                    <div className="flex gap-2">
                        ...
                        {/* <Button
                            type="default"
                            className={`flex-1 ${skin === 'dark' ? 'border-gray-600 text-gray-300' : ''}`}
                        >
                            Unfriend
                        </Button> */}
                    </div>
        ...
        <>
            ...
            >
                <div className="space-y-3">
                    {suggestFriends?.slice(0, 5)?.map((item) => (
                        <FriendItem key={item?.details?.documentId} item={item} />
                    ))}

                    {suggestFriends?.length > 5 && (
            ...
            >
                <div className="friend-list max-h-[70vh] overflow-y-auto px-2">
                    <div className="grid grid-cols-2 gap-4">
                        {suggestFriends?.filter(item => item?.details?.documentId !== userData?.documentId)?.map((item) => (
